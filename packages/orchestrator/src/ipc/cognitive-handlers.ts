...
export interface CognitiveHandlerDependencies {
    ...
    memoryStore?: {
        searchMemories: (query: string, limit?: number) => Array<{ content: string; score: number; metadata?: Record<string, unknown> }>;
        storeMemory: (memory: { chatId: number; text: string; sender: string; messageId?: number }) => Promise<void>;
 ...
 */
function toCognitiveStateSnapshot(state: any, emotionalState: any): CognitiveStateSnapshot {
    return {
...
export function registerCognitiveHandlers(
    ...
    ipcServer.registerHandler(
        ...
        async (payload: CognitiveProcessRequest): Promise<CognitiveProcessResponse> => {
            ...
            const result = await cognitiveOrchestrator.processMessage(payload.message, {
                ...
                response: {
                    ...
                    metadata: {
                        sentiment: sentiment ? {
                            ...
                            label: sentiment.polarity > 0 ? 'positive' : sentiment.polarity < 0 ? 'negative' : 'neutral',
                        } : undefined,
                        emotion: emotion ? {
    ...
    ipcServer.registerHandler(
        ...
        async (payload: CognitiveQuickProcessRequest): Promise<{ response: string }> => {
            ...
            const response = await cognitiveOrchestrator.quickProcess(payload.message);
            return { response };
        }
    );
