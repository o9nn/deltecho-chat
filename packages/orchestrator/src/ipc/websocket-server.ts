...
export class WebSocketServer extends EventEmitter {
        ...
        return new Promise((resolve, reject) => {
            try {
                ...
                this.httpServer.listen(this.config.port, this.config.host, () => {
                    log.info(`WebSocket server listening on ${this.config.useHttps ? 'wss' : 'ws'}://${this.config.host}:${this.config.port}${this.config.path}`);
                    this.running = true;
                    this.startHeartbeat();
                    resolve();
                });
    ...
    private handleUpgrade(request: http.IncomingMessage, socket: any, head: Buffer): void {
        ...
        socket.write('\r\n');
        socket.write('WebSocket requires ws package. Install with: pnpm add ws @types/ws\r\n');
        socket.destroy();
    }
     ...
     */
    private async handleMessage(client: WebSocketClient, message: IPCMessage): Promise<void> {
        ...
        if (!handler) {
            this.sendError(client.ws, message.id, `Unknown message type: ${message.type}`);
            return;
        }
