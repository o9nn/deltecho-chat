...
import type { AARState, AgentState, ArenaState, RelationState } from './types.js';
...
export class AARPersistence extends EventEmitter {
     ...
     */
    startAutoSave(
        getState: () => { agent: AgentMembrane; arena: ArenaMembrane; relation: RelationInterface; cycle: number; instanceName: string }
    ): void {
        ...
        log.info(`Auto-save started (interval: ${this.config.autoSaveIntervalMs}ms)`);
    }
    ...
    async load(): Promise<{
        ...
        try {
            ...
            const meta = await this.readStateFile(STATE_FILES.meta) as PersistenceMeta;
            ...
            log.info(`State loaded (cycle ${meta.cycle}, saved ${new Date(meta.lastSaved).toISOString()})`);
