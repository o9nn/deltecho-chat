...
function createDefaultPhases(): NarrativePhases {
    ...
    return {
        origin: createPhase(0, 'Origin', 'past', ['history', 'roots', 'foundations']),
        journey: createPhase(1, 'Journey', 'past', ['experience', 'learning', 'growth']),
        arrival: createPhase(2, 'Arrival', 'past', ['transition', 'culmination', 'achievement']),
        situation: createPhase(3, 'Situation', 'present', ['context', 'circumstances', 'setting']),
        engagement: createPhase(4, 'Engagement', 'present', ['dialogue', 'interaction', 'connection']),
        culmination: createPhase(5, 'Culmination', 'present', ['resolution', 'synthesis', 'insight']),
        possibility: createPhase(6, 'Possibility', 'future', ['potential', 'options', 'imagination']),
        trajectory: createPhase(7, 'Trajectory', 'future', ['direction', 'momentum', 'purpose']),
        destiny: createPhase(8, 'Destiny', 'future', ['vision', 'aspiration', 'transcendence']),
    };
}
...
export class ArenaMembrane extends EventEmitter {
    ...
    getActiveFrames(): SessionFrame[] {
        return Array.from(this.state.activeFrames.values())
    ...
    transitionPhase(
        ...
        log.debug(`Phase transition: ${oldPhase} → ${phaseName} (${intensity.toFixed(2)})`);
    }
    ...
    getTemporalFocus(): { past: number; present: number; future: number } {
        ...
        return {
            past: (phases.origin.intensity + phases.journey.intensity + phases.arrival.intensity) / 3,
            present: (phases.situation.intensity + phases.engagement.intensity + phases.culmination.intensity) / 3,
            future: (phases.possibility.intensity + phases.trajectory.intensity + phases.destiny.intensity) / 3,
        };
    }
    ...
    incrementMessageCount(frameId?: string): void {
        const frame = this.state.activeFrames.get(frameId || this.state.currentFrameId);
        if (frame) {
            frame.messageCount++;
            frame.lastActiveAt = Date.now();
        }
     ...
     */
    addLore(entry: Omit<LoreEntry, 'id' | 'createdAt' | 'accessCount'>): LoreEntry {
        ...
        if (this.state.yggdrasilReservoir.length > this.maxLoreEntries) {
            this.state.yggdrasilReservoir.sort((a, b) =>
                (b.weight * (1 + b.accessCount * 0.1)) -
                (a.weight * (1 + a.accessCount * 0.1))
            );
            this.state.yggdrasilReservoir = this.state.yggdrasilReservoir.slice(0, this.maxLoreEntries);
        }
    ...
    getLoreStats(): {
        ...
        return {
            ...
            byCategory,
            avgWeight: this.state.yggdrasilReservoir.length > 0
    ...
    private extractLoreFromFrame(frame: SessionFrame): void {
        ...
        if (frame.messageCount >= 5) {
            this.addLore({
                category: 'story',
                content: `Session "${frame.title}" with ${frame.participants.join(', ')}: ` +
                    `${frame.messageCount} exchanges, themes: ${frame.narrativeContext.thematicElements.join(', ')}`,
                sourceFrameId: frame.frameId,
                contributors: frame.participants,
                weight: Math.min(1, frame.messageCount / 20),
                tags: [...frame.narrativeContext.thematicElements, ...frame.narrativeContext.storyThreads],
                connections: [],
            });
        }
    }
