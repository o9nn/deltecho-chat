 ...
 */
const FACET_TO_EMOTION_MAP: Record<keyof CharacterFacets, { emotion: string; weight: number }[]> = {
    wisdom: [
 ...
 */
const EMOTION_TO_FACET_MAP: Record<string, { facet: keyof CharacterFacets; weight: number }[]> = {
    joy: [
...
export class AARPersonaBridge {
    ...
    private async syncAARToPersona(): Promise<void> {
        ...
        for (const [facetName, facet] of Object.entries(agentState.facets)) {
            const mappings = FACET_TO_EMOTION_MAP[facetName as keyof CharacterFacets] || [];

            for (const mapping of mappings) {
                const delta = (facet.activation - 0.5) * mapping.weight * this.config.aarToPersonaWeight;

                // Check if it's a cognitive parameter or emotional
                if (['certainty', 'curiosity', 'creativity', 'focus', 'reflection'].includes(mapping.emotion)) {
                    cognitiveDeltas[mapping.emotion] = (cognitiveDeltas[mapping.emotion] || 0) + delta;
                } else {
                    emotionDeltas[mapping.emotion] = (emotionDeltas[mapping.emotion] || 0) + delta;
                }
            }
        }
    ...
    private async syncPersonaToAAR(): Promise<void> {
        ...
        for (const [emotion, value] of Object.entries(emotionalState)) {
            ...
            for (const mapping of mappings) {
                const adjustment = (value - 0.5) * mapping.weight * this.config.personaToAARWeight;
                facetAdjustments[mapping.facet] = (facetAdjustments[mapping.facet] || 0) + adjustment;
            }
        }
        ...
        const vadMapping = this.emotionToVAD(dominantEmotion.emotion, dominantEmotion.intensity);
     ...
     */
    private emotionToVAD(emotion: string, intensity: number): {
        valence?: number;
        arousal?: number;
        dominance?: number;
    } {
        const vadMap: Record<string, { valence: number; arousal: number; dominance: number }> = {
