...
export class AARSystem extends EventEmitter {
    ...
    constructor(config: Partial<AARConfig> = {}) {
        ...
        this.arena = new ArenaMembrane({ maxLoreEntries: this.config.maxLoreEntries });
        this.relation = new RelationInterface({ maxFlowHistory: this.config.maxFlowHistory });
    ...
    private syncCycle(): void {
        ...
        const overallCoherence = (agentCoherence + arenaCoherence + relationCoherence) / 3;
        ...
        if (Math.abs(overallCoherence - relationCoherence) > 0.1) {
            this.emitAAREvent('aar:coherence-change', {
                ...
                components: { agent: agentCoherence, arena: arenaCoherence, relation: relationCoherence },
            });
        }
    ...
    async processMessage(context: MessageContext): Promise<AARProcessingResult> {
        ...
        log.debug(`Processing message from ${context.senderName}: ${context.content.slice(0, 50)}...`);
        ...
        this.agent.updateSocialMemory(context.senderId, {
            ...
            familiarity: Math.min(1, (this.agent.getSocialMemory(context.senderId)?.familiarity || 0) + 0.05),
        });
        ...
        return {
            ...
            arenaState: {
                currentPhase: Object.entries(arenaState.phases)
                    .sort(([, a], [, b]) => b.intensity - a.intensity)[0]?.[0] || 'engagement',
    ...
    private ensureFrame(context: MessageContext): SessionFrame {
        ...
        const chatFrame = existingFrames.find(f =>
    ...
    private analyzeEmotionalContent(content: string): {
        ...
        const positiveWords = ['happy', 'good', 'great', 'love', 'wonderful', 'amazing', 'thanks', 'thank', 'awesome', 'excited'];
        const negativeWords = ['sad', 'bad', 'terrible', 'hate', 'awful', 'angry', 'frustrated', 'upset', 'worried', 'anxious'];
        const highArousalWords = ['excited', 'urgent', 'important', 'amazing', 'incredible', '!', 'help', 'now'];
        const lowArousalWords = ['calm', 'peaceful', 'relaxed', 'quiet', 'gentle', 'slowly'];
        ...
        if (positiveCount > 0 || negativeCount > 0) {
            result.valence = (positiveCount - negativeCount) / Math.max(positiveCount + negativeCount, 1);
        }
    ...
    private inferNarrativePhase(content: string): keyof NarrativePhases {
        ...
        // Past-oriented
        if (lower.includes('remember') || lower.includes('used to') || lower.includes('before')) {
            return 'journey';
        }

        // Future-oriented
        if (lower.includes('will') || lower.includes('going to') || lower.includes('tomorrow') ||
            lower.includes('plan') || lower.includes('hope')) {
            return 'possibility';
        }
        ...
        // Crisis/urgent
        if (lower.includes('help') || lower.includes('urgent') || lower.includes('need')) {
            return 'culmination';
        }
    ...
    private inferFacetsFromContent(content: string): (keyof CharacterFacets)[] {
        ...
        if (lower.includes('help') || lower.includes('support') || lower.includes('feeling')) {
            facets.push('compassion');
        }

        if (lower.includes('?') || lower.includes('how') || lower.includes('what') ||
            lower.includes('why') || lower.includes('learn')) {
            facets.push('curiosity');
        }

        if (lower.includes('think') || lower.includes('understand') || lower.includes('explain')) {
            facets.push('wisdom');
        }

        if (lower.includes('haha') || lower.includes('lol') || lower.includes('funny') ||
            lower.includes('joke') || lower.includes('play')) {
            facets.push('playfulness');
        }

        if (lower.includes('honest') || lower.includes('really') || lower.includes('truth')) {
            facets.push('authenticity');
        }

        if (lower.includes('goal') || lower.includes('achieve') || lower.includes('must') ||
            lower.includes('need to')) {
            facets.push('determination');
        }

        if (lower.includes('careful') || lower.includes('warn') || lower.includes('safe')) {
            facets.push('protector');
        }

        if (lower.includes('spiritual') || lower.includes('meaning') || lower.includes('connected') ||
    ...
    getContextSummary(): string {
        ...
        const activePhase = Object.entries(arenaState.phases)
            .sort(([, a], [, b]) => b.intensity - a.intensity)[0]?.[0] || 'engagement';

        return `[AAR Context]
Self-Narrative: ${relationState.selfReflection.selfNarrative}
Character: ${dominantFacet}-oriented (activation: ${agentState.facets[dominantFacet].activation.toFixed(2)})
Emotional State: valence=${emotionalState.valence.toFixed(2)}, arousal=${emotionalState.arousal.toFixed(2)}
