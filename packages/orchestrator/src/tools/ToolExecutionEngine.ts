...
export class ToolExecutionEngine extends EventEmitter {
  ...
  async execute(call: ToolCall): Promise<ToolResult> {
    ...
    log.info(
      `Tool ${call.toolName} completed: success=${result.success}, duration=${result.duration}ms`,
    );
  ...
  private async executeMcp(args: Record<string, unknown>): Promise<unknown> {
    ...
    const command = `manus-mcp-cli tool call ${tool} --server ${server} --input '${inputJson.replace(
      /'/g,
      "'\\''",
    )}'`;
