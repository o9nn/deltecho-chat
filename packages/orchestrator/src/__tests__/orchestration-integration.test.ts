...
describe('Orchestration Integration', () => {
  ...
  beforeEach(() => {
    ...
    sys6Bridge.on('cycle_complete', (result: { telemetry: { processingTimeMs: number } }) => {
  ...
  describe('Stream Coordination', () => {
    it('should maintain triadic stream coherence during operation', async () => {
      ...
      const totalSalience = state.streams.reduce((sum, s) => sum + s.salience, 0);
    ...
    it('should process streams in parallel when enabled', async () => {
      ...
      if (stepTimes.length >= 2) {
        const avgInterval = (stepTimes[stepTimes.length - 1] - stepTimes[0]) / stepTimes.length;
  ...
  describe('Agent Task Coordination', () => {
    ...
    it('should handle task dependencies correctly', async () => {
      ...
      // Create parent task
      const parentTask = agentCoordinator.createTask('parent', 'Parent task', {});

      // Create child tasks
      agentCoordinator.createTask('child1', 'Child 1', {}, 'medium', parentTask.id);
  ...
  describe('Error Handling and Recovery', () => {
    it('should continue operation after component errors', async () => {
      ...
      expect(errorMetric?.dataPoints[errorMetric.dataPoints.length - 1].value).toBe(2);
    });
