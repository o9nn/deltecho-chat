...
export class MemoryAdapter {
    ...
    getRecentContext(count?: number): string[] {
        if (!this.memory) return [];
        return this.memory.retrieveRecentMemories(count ?? this.config.defaultSearchLimit);
    }

    /**
     * Store a message in memory
     */
    async storeMessage(message: UnifiedMessage, chatId: number, messageId: number): Promise<void> {
        if (!this.memory) return;
