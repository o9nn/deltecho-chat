...
export class CognitiveStateManager extends EventEmitter {
    ...
    private updateDominantEmotion(): void {
        ...
        const emotionKeys: (keyof EmotionalVector)[] = [
            'joy', 'sadness', 'anger', 'fear', 'surprise', 'disgust', 'contempt', 'interest'
        ];
    ...
    updateEmotionalState(updates: Partial<EmotionalVector>): void {
        ...
        this.state.lastUpdated = Date.now();
        this.emitEvent('state_updated', { emotionalState: this.state.emotionalState });
    }
    ...
    private recalculateValenceArousal(): void {
        ...
        e.valence = total > 0 ? Math.max(-1, Math.min(1, (positive - negative) / total)) : 0;
        ...
        e.arousal = arousalTotal > 0 ? Math.max(0, Math.min(1, high / (arousalTotal + 1))) : 0.1;
    }

    /**
     * Create a new triadic stream
     */
    createStream(phase: 'sense' | 'process' | 'act', data: unknown): TriadicStream {
    ...
    updateStream(
        streamId: string,
        updates: Partial<Pick<TriadicStream, 'phase' | 'status' | 'data' | 'priority'>>
    ): boolean {
    ...
    getActiveStreams(): TriadicStream[] {
        return this.state.activeStreams.filter(s => s.status === 'active' || s.status === 'pending');
    }
    ...
    private pruneStreams(): void {
        ...
        while (this.state.activeStreams.length >= this.config.maxStreams) {
            const pending = this.state.activeStreams.filter(s => s.status === 'pending');
            if (pending.length > 0) {
                pending.sort((a, b) => a.timestamp - b.timestamp);
                const oldestId = pending[0].id;
                this.state.activeStreams = this.state.activeStreams.filter(s => s.id !== oldestId);
            } else {
                break;
            }
        }
