...
import { SentimentAnalyzer, createSentimentAnalyzer } from './sentiment-analyzer';
...
export class CognitiveOrchestrator extends EventEmitter {
    ...
    async processMessage(
        ...
        if (this.config.enableSentiment && !options.skipSentiment) {
            ...
            userMessage = this.messageHandler.enrichWithSentiment(userMessage, sentiment);
        ...
        let memoryContext = '';
        if (this.config.enableMemory && !options.skipMemory && this.memoryAdapter.isEnabled()) {
            const memoryStart = Date.now();
            const memories = this.memoryAdapter.searchRelevant(content, this.config.memorySearchLimit);
            ...
            if (memories.length > 0) {
                ...
                userMessage = this.messageHandler.enrichWithMemories(userMessage, memoryIds);
        ...
        metrics.tokensProcessed = llmResponse.tokens ?? Math.ceil(llmResponse.content.length / 4);
        ...
        let responseMessage = this.messageHandler.createAssistantMessage(llmResponse.content);

        // Analyze response sentiment
        if (this.config.enableSentiment) {
            const responseSentiment = this.sentimentAnalyzer.analyze(llmResponse.content);
            const responseEmotion = this.sentimentAnalyzer.extractEmotion(llmResponse.content);
            responseMessage = this.messageHandler.enrichWithSentiment(responseMessage, responseSentiment);
            responseMessage = this.messageHandler.enrichWithEmotion(responseMessage, responseEmotion);
        }
        ...
        if (this.config.enableMemory && this.memoryAdapter.isConnected()) {
            ...
            await this.memoryAdapter.storeMessage(responseMessage, chatId, messageId + 1);
        }
     ...
     * Get sentiment trend
     */
    getSentimentTrend(): { direction: 'positive' | 'negative' | 'stable'; strength: number } {
     ...
     */
    private emitEvent(type: CognitiveEvent['type'], data?: unknown, messageId?: string): void {
    ...
    getStatistics(): {
        ...
        return {
            ...
            averageProcessingTime: stats.timeSpan > 0
                ? stats.timeSpan / Math.max(1, stats.totalMessages)
                : 0,
            currentCognitiveLoad: state.cognitiveLoad,
            activeStreams: state.activeStreams.filter(s => s.status === 'active').length,
        };
    }
}
