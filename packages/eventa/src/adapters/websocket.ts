...
export function createWebSocketAdapter(
    ...
    function connect(): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            socket.onclose = () => {
                if (!closed && autoReconnect && reconnectAttempts < maxReconnectAttempts) {
                    reconnectAttempts++;
                    console.log(
 ...
 */
export function createWebSocketServerAdapter(
    wss: { on: (event: string, handler: (socket: WebSocket) => void) => void }
): {
    onConnection: (handler: (adapter: TransportAdapter, socket: WebSocket) => void) => void;
    broadcast: (message: WireMessage) => void;
    close: () => void;
} {
    const clients = new Set<WebSocket>();
    const handlers = new Set<(adapter: TransportAdapter, socket: WebSocket) => void>();
    ...
    wss.on('connection', (socket: WebSocket) => {
        ...
        const adapter: TransportAdapter = {
            ...
            onMessage(handler: (message: WireMessage) => void): Subscription {
                ...
                (socket as unknown as { on: (event: string, handler: (data: string) => void) => void }).on(
                    'message',
                    listener
                );
