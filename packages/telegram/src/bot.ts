...
export class DeepTreeEchoTelegramBot {
     ...
     * Handle command
     */
    private async handleCommand(ctx: Context, commandName: string): Promise<void> {
    ...
    private async handleTextMessage(ctx: Context): Promise<void> {
        ...
        if (this.config.username) {
            content = content.replace(new RegExp(`@${this.config.username}`, 'gi'), '').trim();
        }

        this.log(`Processing message from ${messageContext.firstName}: "${content.substring(0, 50)}..."`);
     ...
     */
    private shouldRespond(context: TelegramMessageContext, text: string): boolean {
 ...
 */
export function createTelegramBot(config: TelegramBotConfig): DeepTreeEchoTelegramBot {
