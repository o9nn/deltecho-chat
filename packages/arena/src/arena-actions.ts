...
export class ArenaActions {
  ...
  extract(objId: string, property: keyof MaterialProperties): ActionResult {
    return this.withCoherenceDelta(2, "spatial_structure", [objId], () => {
      ...
      const extracted: ArenaObject = {
        ...
        simplex: { level: 1, volume: 0.1, surface: 0.2, edge: 0.5, vertex: 0.1 },
        aesthetic: { centrality: 0.2, radiance: 0.3, patina: obj.aesthetic.patina, scaleRelative: 0.05, semanticWeight: "satellite" },
  ...
  merge(objIdA: string, objIdB: string): ActionResult {
    return this.withCoherenceDelta(5, "spatial_structure", [objIdA, objIdB], () => {
      ...
      a.simplex.surface = Math.min(1, a.simplex.surface + b.simplex.surface * 0.5);
      a.aesthetic.radiance = Math.min(1, (a.aesthetic.radiance + b.aesthetic.radiance) / 1.5);
  ...
  nest(outerId: string, innerId: string): ActionResult {
    return this.withCoherenceDelta(7, "spatial_structure", [outerId, innerId], () => {
  ...
  addBackup(objId: string): ActionResult {
    return this.withCoherenceDelta(11, "force_and_field", [objId], () => {
      ...
      const backup: ArenaObject = {
        ...
        aesthetic: { ...obj.aesthetic, radiance: obj.aesthetic.radiance * 0.5, semanticWeight: "satellite" },
      };
  ...
  reframeHarm(objId: string): ActionResult {
    return this.withCoherenceDelta(22, "material_and_substance", [objId], () => {
      const obj = this.grid.getObject(objId);
      if (!obj) return false;

      // Convert high reactivity into useful energy
      if (obj.material.reactivity > 0.5) {
        obj.aesthetic.radiance = Math.min(1, obj.aesthetic.radiance + obj.material.reactivity * 0.5);
  ...
  addFeedback(outputId: string, inputId: string): ActionResult {
    return this.withCoherenceDelta(23, "material_and_substance", [outputId, inputId], () => {
  ...
  mediate(aId: string, bId: string): ActionResult {
    return this.withCoherenceDelta(24, "material_and_substance", [aId, bId], () => {
      ...
      const mediator: ArenaObject = {
        ...
        simplex: { level: 1, volume: 0.2, surface: 0.4, edge: 0.3, vertex: 0.1 },
        aesthetic: { centrality: 0.5, radiance: 0.4, patina: 0, scaleRelative: 0.1, semanticWeight: "connector" },
        material: { cost: 0.2, durability: 0.8, replaceability: 0.9, flexibility: 0.7, porosity: 0.5, reactivity: 0, temperature: 0, phase: "membrane" },
  ...
  selfServe(objId: string): ActionResult {
    return this.withCoherenceDelta(25, "material_and_substance", [objId], () => {
      const obj = this.grid.getObject(objId);
      if (!obj) return false;
  ...
  copy(objId: string): ActionResult {
    return this.withCoherenceDelta(26, "material_and_substance", [objId], () => {
      const obj = this.grid.getObject(objId);
      if (!obj) return false;
      ...
      const replica: ArenaObject = {
        ...
        name: `${obj.name}_copy`,
        position: hexNeighbors(obj.position)[0] || obj.position,
        material: { ...obj.material, cost: obj.material.cost * 0.3, durability: obj.material.durability * 0.5 },
        aesthetic: { ...obj.aesthetic, patina: 0, radiance: obj.aesthetic.radiance * 0.6 },
      };
  ...
  makeDisposable(objId: string): ActionResult {
    return this.withCoherenceDelta(27, "material_and_substance", [objId], () => {
      const obj = this.grid.getObject(objId);
      if (!obj) return false;
  ...
  homogenize(center: HexCoord, radius: number): ActionResult {
    const affected = this.grid.objectsInRange(center, radius).map(o => o.id);
    return this.withCoherenceDelta(33, "system_transformation", affected, () => {
      const objects = this.grid.objectsInRange(center, radius);
  ...
  changeParameter(objId: string, param: "temperature" | "pressure" | "reactivity", value: number): ActionResult {
    return this.withCoherenceDelta(35, "environmental_interaction", [objId], () => {
      const obj = this.grid.getObject(objId);
      if (!obj) return false;
  ...
  exploitTransition(objId: string): ActionResult {
    return this.withCoherenceDelta(36, "environmental_interaction", [objId], () => {
      const obj = this.grid.getObject(objId);
      if (!obj) return false;

      // Phase transition releases energy
      const phases: MaterialProperties["phase"][] = ["solid", "fluid", "membrane", "field"];
  ...
  expandDifferentially(objId: string): ActionResult {
    return this.withCoherenceDelta(37, "environmental_interaction", [objId], () => {
      const obj = this.grid.getObject(objId);
      if (!obj) return false;

      if (obj.material.temperature > 0) {
        obj.simplex.volume = Math.min(1, obj.simplex.volume * (1 + obj.material.temperature * 0.3));
  ...
  catalyze(objId: string, catalystStrength: number): ActionResult {
    return this.withCoherenceDelta(38, "environmental_interaction", [objId], () => {
      const obj = this.grid.getObject(objId);
      if (!obj) return false;

      obj.material.reactivity = Math.min(1, obj.material.reactivity + catalystStrength);
      obj.aesthetic.radiance = Math.min(1, obj.aesthetic.radiance + catalystStrength * 0.3);
  ...
  protect(objId: string): ActionResult {
    return this.withCoherenceDelta(39, "environmental_interaction", [objId], () => {
      const obj = this.grid.getObject(objId);
      if (!obj) return false;
  ...
  compose(objIdA: string, objIdB: string): ActionResult {
    return this.withCoherenceDelta(40, "environmental_interaction", [objIdA, objIdB], () => {
      const a = this.grid.getObject(objIdA);
      const b = this.grid.getObject(objIdB);
      if (!a || !b) return false;
      ...
      a.material.durability = Math.max(a.material.durability, b.material.durability);
      a.material.flexibility = Math.max(a.material.flexibility, b.material.flexibility);
  ...
  private withCoherenceDelta(
    ...
    const coherenceAfter = success ? this.measureGlobalCoherence() : coherenceBefore;
