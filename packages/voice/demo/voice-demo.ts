 ...
 */
const VISEME_SHAPES: Record<string, { width: number; height: number; borderRadius: string }> = {
...
class VoiceDemoController {
    ...
    private getElements() {
        return {
            ...
            transcriptionContent: document.getElementById('transcription-content') as HTMLElement,
            transcriptionInterim: document.getElementById('transcription-interim') as HTMLElement,
            ...
            emotionConfidence: document.getElementById('emotion-confidence') as HTMLElement,
            ttsText: document.getElementById('tts-text') as HTMLTextAreaElement,
            emotionSelect: document.getElementById('emotion-select') as HTMLSelectElement,
            ...
            timelineProgress: document.getElementById('timeline-progress') as HTMLElement,
     ...
     */
    private handleRecognitionResult(result: { text: string; confidence: number; isFinal: boolean }): void {
        if (result.isFinal) {
            this.transcriptionText += (this.transcriptionText ? ' ' : '') + result.text;
    ...
    private updateEmotionDisplay(emotion: string, confidence: number): void {
        ...
        this.elements.emotionName.textContent = emotion.charAt(0).toUpperCase() + emotion.slice(1);
        this.elements.emotionConfidence.textContent = `${Math.round(confidence * 100)}%`;
    }
    ...
    private displayLipSyncTimeline(data: LipSyncData): void {
        ...
        data.visemes.forEach((viseme) => {
            ...
            const widthPercent = ((viseme.endTime - viseme.startTime) / totalDuration) * 100;
    ...
    private animateVisemes(data: LipSyncData): void {
        ...
        const animate = () => {
            ...
            if (progress < 1) {
                ...
                this.updateViseme({ viseme: 'sil', startTime: 0, endTime: 0, phoneme: '' });
            }
        };
    ...
    private renderVisemeHistory(): void {
        this.elements.visemeList.innerHTML = this.visemeHistory
            .map(
                (v, i) => `<span class="viseme-item ${i === 0 ? 'active' : ''}">${v}</span>`
            )
            .join('');
    }
    ...
    private updateStatus(type: string, text: string, state: string): void {
        const statusEl = this.elements[`${type}Status` as keyof typeof this.elements] as HTMLElement;
        const dotEl = this.elements[`${type}StatusDot` as keyof typeof this.elements] as HTMLElement;
