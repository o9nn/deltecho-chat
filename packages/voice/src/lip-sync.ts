 ...
 */
export type LipSyncEventType = 'phoneme_start' | 'phoneme_end' | 'sync_complete' | 'error';
...
export class LipSyncGenerator extends EventEmitter {
    ...
    generateFromText(text: string): LipSyncData {
        ...
        for (const word of words) {
            ...
            if (wordPhonemes.length > 0) {
                ...
                currentTime = lastPhoneme.startTime + lastPhoneme.duration + this.config.wordGapDuration;
            }
        }
     ...
     */
    generateFromAudioLevel(audioLevel: number, duration: number = 100): PhonemeEntry {
    ...
    private updatePlayback(): void {
        ...
        while (
            ...
            elapsed >= phonemes[this.currentIndex].startTime + phonemes[this.currentIndex].duration
        ) {
            // End current phoneme
            ...
            if (this.currentIndex < phonemes.length && elapsed >= phonemes[this.currentIndex].startTime) {
                this.emitEvent('phoneme_start', { phoneme: phonemes[this.currentIndex] });
            }
        }
    ...
    getCurrentPhoneme(): PhonemeEntry | null {
        if (!this.currentData || this.currentIndex >= this.currentData.phonemes.length) {
            return null;
        }
 ...
 */
export function createLipSyncGenerator(config?: Partial<LipSyncConfig>): LipSyncGenerator {
...
export function phonemeToMouthShape(entry: PhonemeEntry): {
    ...
    switch (phoneme) {
        case 'A':
            return { mouthOpen: 1.0 * intensity, mouthWide: 0.5 * intensity, lipRound: 0 };
        case 'B':
            return { mouthOpen: 0, mouthWide: 0, lipRound: 0 };
        case 'C':
            return { mouthOpen: 0.4 * intensity, mouthWide: 0.6 * intensity, lipRound: 0 };
        case 'D':
            return { mouthOpen: 0.8 * intensity, mouthWide: 1.0 * intensity, lipRound: 0 };
        case 'E':
            return { mouthOpen: 0.6 * intensity, mouthWide: 0, lipRound: 0.8 * intensity };
        case 'F':
            return { mouthOpen: 0.3 * intensity, mouthWide: 0, lipRound: 1.0 * intensity };
        case 'G':
            return { mouthOpen: 0.2 * intensity, mouthWide: 0.3 * intensity, lipRound: 0 };
        case 'H':
            return { mouthOpen: 0.3 * intensity, mouthWide: 0, lipRound: 0 };
        case 'I':
            return { mouthOpen: 0.3 * intensity, mouthWide: 0.8 * intensity, lipRound: 0 };
        case 'X':
        default:
            return { mouthOpen: 0, mouthWide: 0, lipRound: 0 };
    }
