...
import { LipSyncData } from './lip-sync';
 ...
 */
const EMOTION_PATTERNS: Array<{ pattern: RegExp; emotion: string; intensity: number }> = [
    { pattern: /\b(happy|glad|joy|wonderful|fantastic|amazing)\b/i, emotion: 'happy', intensity: 0.8 },
    { pattern: /\b(sad|sorry|unfortunately|regret)\b/i, emotion: 'sad', intensity: 0.6 },
    { pattern: /\b(excited|exciting|wow|incredible)\b/i, emotion: 'excited', intensity: 0.9 },
    { pattern: /\b(worried|concern|careful|caution)\b/i, emotion: 'concerned', intensity: 0.5 },
    { pattern: /\b(interesting|curious|fascinating)\b/i, emotion: 'curious', intensity: 0.6 },
...
export class StreamingResponseHandler extends EventEmitter {
    ...
    constructor(config: Partial<StreamingHandlerConfig> = {}) {
        ...
        this.lipSyncController = createStreamingLipSyncController(this.config.lipSyncConfig);
    ...
    private setupLipSyncListeners(): void {
        this.lipSyncController.onStreamingLipSyncEvent((event: StreamingLipSyncEvent) => {
            switch (event.type) {
                case 'mouth_update':
    ...
    private handlePhraseReady(phrase: string): void {
        ...
        if (this.config.enableEmotionDetection && this.avatarReceiver?.setExpression) {
    ...
    private async processNextTTSPhrase(): Promise<void> {
        if (this.isTTSSpeaking || !this.ttsProvider || this.ttsPhraseQueue.length === 0) {
            return;
        }
     ...
     */
    private detectEmotion(text: string): { emotion: string; intensity: number } | null {
