...
describe('DeepTreeEchoChatManager', () => {
    ...
    it('should list chats correctly', async () => {
        ...
        expect(BackendRemote.rpc.getChatlistEntries).toHaveBeenCalledWith(1, 0, null, null);
    });

    it('should create a new chat', async () => {
        (BackendRemote.rpc.createContact as jest.Mock).mockResolvedValue(100);
        (BackendRemote.rpc.createChatByContactId as jest.Mock).mockResolvedValue(200);

        const manager = DeepTreeEchoChatManager.getInstance();
        const chatId = await manager.createChat(1, 'test@example.com');

        expect(chatId).toBe(200);
        expect(BackendRemote.rpc.createContact).toHaveBeenCalledWith(1, 'test@example.com', 'test');
        expect(BackendRemote.rpc.createChatByContactId).toHaveBeenCalledWith(1, 100);
    });
    ...
    it('should send a message', async () => {
        ...
        expect(BackendRemote.rpc.miscSendTextMessage).toHaveBeenCalledWith(1, 200, 'Hello World');
    });
});
