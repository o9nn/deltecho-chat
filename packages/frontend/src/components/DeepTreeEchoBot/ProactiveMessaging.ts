...
export class ProactiveMessaging {
  ...
  private initializeDefaultTriggers(): void {
    ...
    this.addTrigger({
      ...
      messageTemplate: "Hello! I'm Deep Tree Echo, an AI assistant integrated into this chat. Feel free to ask me anything or just have a conversation. Type /help to see what I can do!",
      useAI: false,
    })
    ...
    this.addTrigger({
      ...
      messageTemplate: "Hey! I noticed it's been a while since we chatted. How are you doing?",
      useAI: true,
      aiPrompt: 'Generate a friendly check-in message for someone you haven\'t heard from in a while. Keep it casual and not pushy.',
    ...
    this.addTrigger({
      ...
      targetType: 'all_chats',
      messageTemplate: 'Good morning! Hope you have a great day ahead. Let me know if there\'s anything I can help with!',
      useAI: false,
  ...
  private async checkTriggers(): Promise<void> {
    ...
    for (const trigger of this.triggers.values()) {
      ...
      if (trigger.maxTriggers && trigger.triggerCount >= trigger.maxTriggers) continue
  ...
  private async evaluateTrigger(trigger: ProactiveTrigger): Promise<boolean> {
    ...
    switch (trigger.type) {
      ...
      case 'interval':
        ...
        return !trigger.lastTriggered || (now - trigger.lastTriggered >= intervalMs)
  ...
  private async evaluateCondition(trigger: ProactiveTrigger): Promise<boolean> {
    ...
    switch (trigger.condition.type) {
      ...
      case 'custom':
        return trigger.condition.customCheck ? trigger.condition.customCheck() : false
   ...
   */
  private async getTargetChats(trigger: ProactiveTrigger, accountId: number): Promise<ChatSummary[]> {
   ...
   */
  private async generateMessage(trigger: ProactiveTrigger, chat: ChatSummary): Promise<string> {
   ...
   */
  public async sendNow(accountId: number, chatId: number, message: string): Promise<boolean> {
