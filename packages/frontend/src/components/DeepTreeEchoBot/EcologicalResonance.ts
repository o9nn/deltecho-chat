...
import { MetabolicResonance, TemporalRhythm } from '@deltachat-desktop/shared/shared-types'
...
export class EcologicalResonance {
    ...
    private tick(): void {
        ...
        const time = Date.now()
        const circadianSine = Math.sin((2 * Math.PI * time) / this.rhythm.periodicity)
        ...
        const surprise = Math.max(0, this.rhythm.interactionDensity - expectedDensity)
    ...
    private calculateEntropy(): number {
        ...
        return this.reservoir.reduce((acc, val) => acc + Math.abs(val), 0) / this.RESERVOIR_SIZE
    }

    public getMetabolicState(): { resonance: MetabolicResonance, rhythm: TemporalRhythm } {
