...
const log = getLogger('render/components/DeepTreeEchoBot/StreamingAvatarService')
 ...
 */
const PHONEME_TO_MOUTH: Record<string, { mouthOpen: number; mouthWide: number; lipRound: number }> = {
...
export class StreamingAvatarService extends EventEmitter {
    ...
    private currentMouthShape: MouthShape = { mouthOpen: 0, mouthWide: 0, lipRound: 0, timestamp: 0 }
    ...
    async generateStreamingResponse(
        ...
        try {
            ...
            log.info(`Streaming generation complete: ${this.tokensReceived} tokens, ${this.fullText.length} chars`)
    ...
    private extractPhrases(): void {
        ...
        if (lastBoundaryIndex >= 0 && lastBoundaryIndex >= this.config.minPhraseLength - 1) {
    ...
    private queuePhrase(text: string): void {
        ...
        const duration = phonemes.reduce((sum, p) => Math.max(sum, p.startTime + p.duration), 0)
    ...
    private generatePhonemes(text: string): PhonemeData[] {
        ...
        for (const char of normalized) {
            if (char === ' ') {
                ...
                const mapping = CHAR_TO_PHONEME[char] || { phoneme: 'H', intensity: 0.5 }
    ...
    private startNextPhrase(): void {
        ...
        log.debug(`Speaking phrase ${this.currentPhraseIndex + 1}/${this.phraseQueue.length}`)
    }
    ...
    private updateAnimation(): void {
        ...
        if (!currentPhrase || currentPhrase.status !== 'speaking') {
            // Return to rest
            this.targetMouthShape = { mouthOpen: 0, mouthWide: 0, lipRound: 0, timestamp: Date.now() }
            this.applySmoothing()
            return
        }
        ...
        for (const phoneme of currentPhrase.phonemes) {
            if (elapsed >= phoneme.startTime && elapsed < phoneme.startTime + phoneme.duration) {
        ...
        if (currentPhoneme) {
            const mouthBase = PHONEME_TO_MOUTH[currentPhoneme.phoneme] || PHONEME_TO_MOUTH['X']
    ...
    private applySmoothing(): void {
        if (this.config.enableSmoothing) {
            ...
            this.currentMouthShape = {
                mouthOpen: this.lerp(this.currentMouthShape.mouthOpen, this.targetMouthShape.mouthOpen, f),
                mouthWide: this.lerp(this.currentMouthShape.mouthWide, this.targetMouthShape.mouthWide, f),
                lipRound: this.lerp(this.currentMouthShape.lipRound, this.targetMouthShape.lipRound, f),
                timestamp: Date.now(),
            }
        } else {
    ...
    private reset(): void {
        ...
        this.currentMouthShape = { mouthOpen: 0, mouthWide: 0, lipRound: 0, timestamp: 0 }
     ...
     * Add event listener
     */
    onStreamingAvatarEvent(listener: (event: StreamingAvatarEvent) => void): void {
     ...
     */
    offStreamingAvatarEvent(listener: (event: StreamingAvatarEvent) => void): void {
     ...
     * Emit event
     */
    private emitEvent(type: StreamingAvatarEventType, data?: StreamingAvatarEvent['data']): void {
