...
import { InfrastructureLatentState, JournalType } from '@deltachat-desktop/shared/shared-types'
        ...
        <div className={styles.monitorContainer}>
            ...
            <div className={styles.statsGrid}>
                <div className={styles.statCard}>
                    ...
                    <div className={`${styles.statValue} ${getEnergyClass(state.freeEnergy)}`}>
                        {(state.freeEnergy * 100).toFixed(1)}%
                    </div>
                    <div className={styles.progressTrack}>
                        <div
                            className={`${styles.progressBar} ${getEnergyClass(state.freeEnergy)}`}
                ...
                <div className={styles.statCard}>
                    ...
                    <div className={styles.progressTrack}>
                        <div
                            ...
                            style={{ width: `${state.globalCoherence * 100}%`, backgroundColor: 'var(--color-primary)' }}
                        />
                    </div>
                </div>
            </div>
            ...
            {state.resonance && state.rhythm && (
                <div className={styles.resonanceSection}>
                    <div className={styles.listHeader}>Ecological Resonance (ESN Reservoir)</div>
                    <div className={styles.resonanceGrid}>
                        <div className={styles.subStat}>
                            <div className={styles.subLabel}>Metabolic Rate (Pacing)</div>
                            <div className={styles.subValue}>{(1 / state.resonance.rateLimitFactor).toFixed(2)}x</div>
                        </div>
                        <div className={styles.subStat}>
                            <div className={styles.subLabel}>Circadian Phase</div>
                            <div className={styles.subValue}>{state.rhythm.phaseOffset > 0 ? 'DIURNAL' : 'NOCTURNAL'}</div>
                        </div>
                    </div>
            ...
            {state.journals && (
                <div className={styles.journalContainer}>
                    <div className={styles.journalHeader}>
                        <div className={styles.listHeader}>Internal Journals (Cognitive Streams)</div>
                        <button
                            className={styles.triggerButton}
                            onClick={() => {
                                internalJournalManager.addEntry('diary', 'Manual Introspection Pulse: User requested a real-time cognitive snapshot.', ['manual-trigger', 'introspection'])
                            }}
                        >
                    ...
                    <div className={styles.journalTabs}>
                        {(['dream', 'learning', 'diary', 'project'] as JournalType[]).map(type => (
                            <button
                                key={type}
                                className={`${styles.journalTab} ${activeJournal === type ? styles.active : ''}`}
                    ...
                    <div className={styles.journalContent}>
                        {state.journals[activeJournal].entries.length === 0 ? (
                            <div className={styles.emptyJournal}>No entries in this stream yet. Resonance integrating...</div>
                        ) : (
                            state.journals[activeJournal].entries.slice(0, 15).map((entry, i) => (
            ...
            <div className={styles.activeShadowsList} style={{ marginTop: '20px' }}>
                <div className={styles.listHeader}>Infrastructure Interface Shadows</div>
                    ...
                    <div key={id} className={styles.shadowItem}>
                        ...
                        <div className={styles.shadowMetrics}>
                            <div className={styles.divergenceBar}>
                                <div
                                    className={`${styles.divergenceFill} ${getEnergyClass(shadow.divergence)}`}
                                    style={{ width: `${shadow.divergence * 100}%` }}
                                />
                            </div>
                            {shadow.learnedPattern && <span className={styles.convergedBadge}>CONVERGED</span>}
                        </div>
                    </div>
                ))}
            </div>
