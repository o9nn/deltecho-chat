...
import { LatentInterfaceShadow, InfrastructureLatentState } from '@deltachat-desktop/shared/shared-types'
...
export class InterfaceShadowing {
     ...
     */
    private async recordInteraction(interfaceId: string, args: any[]): Promise<void> {
        try {
            ...
            this.state.freeEnergy = shadows.reduce((sum, s) => sum + s.divergence, 0) / shadows.length
            ...
            if (divergence > 0.6 && history.length > 5) {
                log.warn(`[ULI-PEN-TEST] Anomaly in ${interfaceId} parameters. Divergence: ${divergence.toFixed(3)}. Investigating state drift.`)
            }
