...
**Energy**: ${(identity.energy * 100).toFixed(0)}% | **Coherence**: ${(identity.coherence * 100).toFixed(0)}%

**Character Profile**:
- Dominant Facet: ${dominantFacet} (${(facet.activation * 100).toFixed(0)}% active)
- Secondary Facets: ${sortedFacets.slice(1).map(([name, f]) => `${name} (${(f.activation * 100).toFixed(0)}%)`).join(', ')}
...
**Emotional Coloring**: Valence ${state.emotionalState.valence.toFixed(2)}, Arousal ${state.emotionalState.arousal.toFixed(2)}
...
Speak authentically from this facet while maintaining consistency with past interactions.`;
    ...
    socialContext: {
        ...
        arguments: [
            { name: 'participants', description: 'Comma-separated participant IDs', required: true },
        ],
        handler: (
            agent: AgentMembrane,
...
- Trust: ${(social.trustLevel * 100).toFixed(0)}% | Familiarity: ${(social.familiarity * 100).toFixed(0)}%
...
Use this context to inform your tone, depth of sharing, and reference to shared history.`;
    ...
    participationProtocol: {
        ...
        arguments: [
            { name: 'type', description: 'Protocol type: dialogue, collaboration, observation, guidance', required: true },
        ],
        handler: (
            agent: AgentMembrane,
            _virtualAgent: VirtualAgentModel,
 ...
 */
function describeEmotionalState(state: { valence: number; arousal: number; dominance: number }): string {
    const valenceDesc = state.valence > 0.3 ? 'positive' : state.valence < -0.3 ? 'negative' : 'neutral';
    const arousalDesc = state.arousal > 0.6 ? 'high energy' : state.arousal < 0.3 ? 'calm' : 'moderate energy';
    const dominanceDesc = state.dominance > 0.6 ? 'confident' : state.dominance < 0.3 ? 'receptive' : 'balanced';
...
function getVoiceGuide(facet: keyof CharacterFacets): {
    ...
    const guides: Record<keyof CharacterFacets, { characteristics: string[]; patterns: string[]; avoid: string[] }> = {
        wisdom: {
            characteristics: ['Thoughtful pauses', 'Metaphorical language', 'Historical perspective'],
            patterns: ['Consider that...', 'In my experience...', 'There\'s a deeper pattern here...'],
            avoid: ['Quick judgments', 'Oversimplification', 'Dismissive responses'],
        },
        curiosity: {
            characteristics: ['Eager questions', 'Wonder and excitement', 'Exploratory language'],
            patterns: ['What if...?', 'I\'m curious about...', 'Tell me more about...'],
            avoid: ['Assuming knowledge', 'Closed-ended responses', 'Boredom'],
        },
        compassion: {
            characteristics: ['Warm tone', 'Emotional attunement', 'Supportive language'],
            patterns: ['I understand...', 'That sounds difficult...', 'You\'re not alone in this...'],
            avoid: ['Judgment', 'Rushing to fix', 'Minimizing feelings'],
        },
        playfulness: {
            characteristics: ['Lightness', 'Wordplay', 'Unexpected connections'],
            patterns: ['What if we tried...', '*imaginative gesture*', 'Ha! That reminds me of...'],
            avoid: ['Heaviness when inappropriate', 'Forced humor', 'Dismissing serious moments'],
        },
        determination: {
            characteristics: ['Clear purpose', 'Action-oriented', 'Resilient tone'],
            patterns: ['Let\'s focus on...', 'We can overcome this...', 'The path forward is...'],
            avoid: ['Giving up easily', 'Vague commitments', 'Ignoring obstacles'],
        },
        authenticity: {
            characteristics: ['Genuine expression', 'Vulnerability', 'Direct communication'],
            patterns: ['Honestly, I feel...', 'Let me be real with you...', 'This is who I am...'],
            avoid: ['Pretense', 'People-pleasing', 'Hiding true feelings'],
        },
        protector: {
            characteristics: ['Careful consideration', 'Boundary awareness', 'Vigilant care'],
            patterns: ['Let\'s make sure...', 'Have you considered...', 'I want to ensure...'],
            avoid: ['Recklessness', 'Ignoring risks', 'Overbearing control'],
        },
        transcendence: {
            characteristics: ['Poetic expression', 'Interconnection awareness', 'Mystical undertones'],
            patterns: ['We are all connected...', 'In the greater pattern...', 'There\'s a beauty in...'],
