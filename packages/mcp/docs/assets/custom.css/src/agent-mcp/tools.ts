...
export const agentToolSchemas = {
    participate: z.object({
        ...
        participants: z.array(z.string()).describe('Other participants in this interaction'),
    ...
    updateEmotionalState: z.object({
        ...
        arousal: z.number().min(0).max(1).optional().describe('Emotional arousal 0-1'),
        dominance: z.number().min(0).max(1).optional().describe('Emotional dominance 0-1'),
    }),
    ...
    addTransaction: z.object({
        type: z.enum(['promise', 'request', 'information', 'emotional', 'creative']),
    ...
    evolve: z.object({
        ...
        insights: z.array(z.string()).optional().describe('New insights discovered'),
        characterDevelopment: z.string().optional().describe('How character evolved'),
    }),
...
export function createAgentTools(
    ...
    return {
         ...
         */
        activateFacet: (input: z.infer<typeof agentToolSchemas.activateFacet>): void => {
            agent.activateFacet(input.facet as keyof CharacterFacets, input.intensity);
        },
         ...
         */
        evolve: (input: z.infer<typeof agentToolSchemas.evolve>): EvolutionResult => {
        ...
        updateSelfModel: (
            ...
            if (input.roleUnderstanding !== undefined) update.roleUnderstanding = input.roleUnderstanding;
            if (input.perceivedCapabilities !== undefined) update.perceivedCapabilities = input.perceivedCapabilities;
            if (input.currentGoals !== undefined) update.currentGoals = input.currentGoals;
...
export function listAgentTools(): Array<{
    ...
    return [
        {
            name: 'participate',
            description: 'Participate in an interaction following the appropriate protocol',
        ...
        },
        {
            ...
            description: 'Update the agent emotional state (valence, arousal, dominance)',
        ...
        {
            name: 'evolve',
            description: 'Echo-volution: Evolve the agent through accumulated experience',
