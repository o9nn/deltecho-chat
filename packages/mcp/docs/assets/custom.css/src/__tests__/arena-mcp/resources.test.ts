...
function createMockArenaMembrane(): ArenaMembrane {
    return {
        getState: vi.fn(() => ({
            ...
            coherence: 0.75,
            yggdrasilReservoir: [
                { id: 'lore-1', content: 'Ancient wisdom', category: 'wisdom', tags: ['old'] },
                { id: 'lore-2', content: 'Story element', category: 'story', tags: ['narrative'] },
                { id: 'lore-3', content: 'User pattern', category: 'pattern', tags: ['behavior'] },
            ],
...
describe('Arena Resources', () => {
    ...
    beforeEach(() => {
        ...
        agentRegistry = new Map([
            ...
            ['agent-2', { agentId: 'agent-2', name: 'Agent Two', status: 'dormant', lastActivity: Date.now() }],
        ]);
    });
    ...
    describe('arenaResources', () => {
        ...
        describe('arena://agents', () => {
            it('should return registered agents', () => {
                ...
                const agents = handler(arena, {} as Record<string, never>, agentRegistry);
    ...
    describe('listArenaResources', () => {
        ...
        it('should include frame resources', () => {
            const resources = listArenaResources(arena, agentRegistry);
            const frameResources = resources.filter(r => r.uri.startsWith('arena://frames/'));

            expect(frameResources.length).toBe(2);
            expect(frameResources.some(r => r.uri === 'arena://frames/frame-1')).toBe(true);
            expect(frameResources.some(r => r.uri === 'arena://frames/frame-2')).toBe(true);
        });
