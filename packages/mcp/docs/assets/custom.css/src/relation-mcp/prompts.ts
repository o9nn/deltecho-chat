...
**Active Identity Themes**: ${emergent.activeThemes.join(', ') || 'None defined'}
**Evolution Vector**: ${emergent.evolutionVector}

**Current Tensions** (creative conflicts to hold):
${emergent.tensions.map(t => `- ${t.pole1} ↔ ${t.pole2} (balance: ${t.balance.toFixed(2)})`).join('\n') || '- None identified'}

**Active Questions About Self**:
${selfReflection.activeQuestions.map(q => `- ${q}`).join('\n') || '- None currently'}

**Recent Insights**:
${selfReflection.recentInsights.slice(-3).map(i => `- ${i}`).join('\n') || '- None recent'}
...
Allow tensions to exist as creative polarities rather than problems to solve.`;
    ...
    identityIntegration: {
        ...
        description: 'Template for integrating Agent and Arena into coherent identity',
        handler: (
            relation: RelationInterface,
...
- Emotional State: valence=${agentState.emotionalState.valence.toFixed(2)}, arousal=${agentState.emotionalState.arousal.toFixed(2)}
...
- Narrative Phase: ${activePhase[0]} (${(activePhase[1].intensity * 100).toFixed(0)}%)
...
but from their dynamic, ongoing synthesis through interaction.`;
    ...
    reflexiveAwareness: {
        ...
        handler: (
            ...
            const agentToArena = flows.filter(f => f.direction === 'agent-to-arena').length;
            const arenaToAgent = flows.filter(f => f.direction === 'arena-to-agent').length;
            const bidirectional = flows.filter(f => f.direction === 'bidirectional').length;
...
**Current Reflexive Awareness Level**: ${(state.reflexiveAwareness * 100).toFixed(0)}%
...
- Last Reflection: ${new Date(virtualAgent.selfAwareness.lastReflection).toISOString()}
- Perceived Accuracy: ${(virtualAgent.selfAwareness.perceivedAccuracy * 100).toFixed(0)}%

**Active Bridges**: ${state.activeBridges.join(', ') || 'None active'}

**Divergence from Actual**:
- Estimated Drift: ${(virtualAgent.worldView.divergenceMetrics.estimatedDrift * 100).toFixed(0)}%
- Known Misalignments: ${virtualAgent.worldView.divergenceMetrics.knownMisalignments.join(', ') || 'None known'}
...
- Estimated Coherence: ${(vo.situationalAwareness.estimatedCoherence * 100).toFixed(0)}%
...
- Misalignments: ${vo.divergenceMetrics.knownMisalignments.join(', ') || 'None known'}
