...
export class LifecycleCoordinator extends EventEmitter {
    ...
    private async executePerception(cycleId: number): Promise<DevelopmentalCycleResult> {
        ...
        const dominantPhase = Object.entries(arenaState.phases)
            .sort(([, a], [, b]) => b.intensity - a.intensity)[0];

        if (dominantPhase) {
            // Raise engagement if narrative is active
            const currentEngagement = agent.getState().engagementLevel;
            const newEngagement = Math.min(1, currentEngagement + dominantPhase[1].intensity * 0.1);
    ...
    private async executeEnaction(cycleId: number): Promise<DevelopmentalCycleResult> {
        ...
        const assumedPhase = virtualAgent.worldView.situationalAwareness.assumedNarrativePhase;
