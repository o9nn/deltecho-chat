...
import { createRelationTools, listRelationTools, relationToolSchemas } from './tools.js';
...
export class RelationMCPServer extends EventEmitter {
    ...
    readResource(uri: string): unknown {
        ...
        switch (uri as RelationMCPResourceUri) {
            case 'relation://self-reflection':
                return relationResources['relation://self-reflection'].handler(this.relation);
            case 'relation://flows':
    ...
    async callTool(name: string, args: unknown): Promise<unknown> {
        ...
        const schema = relationToolSchemas[name as keyof typeof relationToolSchemas];
        if (schema) {
    ...
    private syncMirror(): void {
        ...
        this.virtualAgent.selfImage.perceivedDominantFacet = agentState.dominantFacet;

        // Find actual dominant narrative phase
        const actualPhase = Object.entries(arenaState.phases)
            .sort(([, a], [, b]) => b.intensity - a.intensity)[0]?.[0];

        // Update Vo with some lag (subjective perception takes time)
        const currentAssumed = this.virtualAgent.worldView.situationalAwareness.assumedNarrativePhase;
    ...
    private calculateDrift(): number {
        ...
        // Agent drift
        if (agentState.dominantFacet !== this.virtualAgent.selfImage.perceivedDominantFacet) {
            drift += 0.2;
        }
