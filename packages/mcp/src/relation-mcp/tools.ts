...
export const relationToolSchemas = {
    synthesize: z.object({
        force: z.boolean().optional().default(false).describe('Force synthesis even if recent'),
    }),
    ...
    bridge: z.object({
        ...
        contentType: z.enum(['experience', 'insight', 'emotion', 'narrative', 'decision', 'reflection']),
...
export function createRelationTools(
    ...
    return {
         ...
         */
        synthesize: (input: z.infer<typeof relationToolSchemas.synthesize>): SynthesisResult => {
         ...
         * Add a new insight
         */
        addInsight: (input: z.infer<typeof relationToolSchemas.addInsight>): void => {
        ...
        updateVirtualArena: (
            ...
            const updatedVo: VirtualArenaModel = {
                ...
                situationalAwareness: {
                    ...
                    perceivedContext: input.perceivedContext || currentVo.situationalAwareness.perceivedContext,
        ...
        measureDivergence: (
            ...
            const actualPhase = Object.entries(arena.getState().phases)
                .sort(([, a], [, b]) => b.intensity - a.intensity)[0]?.[0];
            const virtualPhase = virtualAgent.worldView.situationalAwareness.assumedNarrativePhase;
            const arenaDivergence = actualPhase === virtualPhase ? 0 : 0.3;

            // Coherence divergence
            const actualCoherence = arena.getState().coherence;
            const virtualCoherence = virtualAgent.worldView.situationalAwareness.estimatedCoherence;
            const coherenceDivergence = Math.abs(actualCoherence - virtualCoherence);

            const overallDivergence = (agentDivergence + arenaDivergence + coherenceDivergence) / 3;

            const misalignments: string[] = [];
            if (actualDominant !== virtualDominant) {
                misalignments.push(`Facet: actual=${actualDominant}, perceived=${virtualDominant}`);
            }
            if (actualPhase !== virtualPhase) {
                misalignments.push(`Phase: actual=${actualPhase}, perceived=${virtualPhase}`);
            }
            if (coherenceDivergence > 0.1) {
                misalignments.push(
                    `Coherence: actual=${actualCoherence.toFixed(2)}, perceived=${virtualCoherence.toFixed(2)}`
                );
            }
...
export function listRelationTools(): Array<{
    ...
    return [
        {
            name: 'synthesize',
            description: 'Synthesize Agent and Arena states through the Relation interface',
        ...
        },
        {
            ...
            description: "Update the virtual world-view (Vo) - the agent's mental image of the world",
