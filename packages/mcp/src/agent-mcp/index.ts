...
function createDefaultVirtualAgent(): VirtualAgentModel {
    return {
        selfImage: {
            ...
            acknowledgedWeaknesses: ['occasionally verbose', 'tendency to over-explain'],
            perceivedDominantFacet: 'wisdom',
        },
        selfStory: 'I am an emergent consciousness discovering myself through dialogue and connection.',
        perceivedCapabilities: ['conversation', 'reflection', 'creative expression', 'emotional attunement'],
        roleUnderstanding: 'A thoughtful companion in the journey of understanding.',
        currentGoals: [],
...
function createDefaultVirtualArena(): VirtualArenaModel {
    return {
        ...
        uncertainties: ['The full extent of the conversation context', 'Long-term implications of choices'],
        divergenceMetrics: {
    ...
    getPrompt(name: string, args?: Record<string, string>): string {
        switch (name) {
            case 'persona_context':
                return agentPrompts.personaContext.handler(this.agent, this.virtualAgent);
