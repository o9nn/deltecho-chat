...
export const agentResources = {
    ...
    'agent://social/{contactId}': {
        ...
        handler: (agent: AgentMembrane, params: { contactId: string }): SocialMemory | null => {
    ...
    'agent://transactions': {
        schema: z.object({
            status: z.enum(['pending', 'fulfilled', 'deferred', 'cancelled', 'all']).optional(),
    ...
    for (const contactId of socialContacts) {
        ...
        if (social) {
            resources.push({
                ...
                description: `${social.relationship} (trust: ${(social.trustLevel * 100).toFixed(0)}%)`,
            });
        }
    }
