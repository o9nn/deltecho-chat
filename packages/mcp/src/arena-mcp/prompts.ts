...
export const arenaPrompts = {
    ...
    worldContext: {
        ...
        handler: (
            ...
            const currentFrame = frames.find(f => f.frameId === state.currentFrameId);

            // Find dominant narrative phase
            const phases = Object.entries(state.phases) as [keyof NarrativePhases, { intensity: number; name: string }][];
            const dominantPhase = phases.sort((a, b) => b[1].intensity - a[1].intensity)[0];
...
**Narrative Phase**: ${dominantPhase[1].name} (${(dominantPhase[1].intensity * 100).toFixed(0)}% intensity)
...
- Narrative Integration: ${(state.gestaltProgress.narrativeIntegration * 100).toFixed(1)}%

**Reservoir Summary**: ${state.yggdrasilReservoir.length} lore entries accumulated`;
        },
    },
    ...
    narrativeWeaving: {
        ...
        handler: (arena: ArenaMembrane): string => {
            ...
            const frames = arena.getActiveFrames();
            const currentFrame = frames.find(f => f.frameId === state.currentFrameId);
            const thematicElements = currentFrame?.narrativeContext.thematicElements || [];
...
- Origin (${(phases.origin.intensity * 100).toFixed(0)}%): ${phases.origin.storyElements.join(', ') || 'Unexplored'}
- Journey (${(phases.journey.intensity * 100).toFixed(0)}%): ${phases.journey.storyElements.join(', ') || 'Unexplored'}
- Arrival (${(phases.arrival.intensity * 100).toFixed(0)}%): ${phases.arrival.storyElements.join(', ') || 'Unexplored'}

**PRESENT (Where we are)**:
- Situation (${(phases.situation.intensity * 100).toFixed(0)}%): ${phases.situation.storyElements.join(', ') || 'Undefined'}
- Engagement (${(phases.engagement.intensity * 100).toFixed(0)}%): ${phases.engagement.storyElements.join(', ') || 'Active'}
- Culmination (${(phases.culmination.intensity * 100).toFixed(0)}%): ${phases.culmination.storyElements.join(', ') || 'Pending'}

**FUTURE (Where we're going)**:
- Possibility (${(phases.possibility.intensity * 100).toFixed(0)}%): ${phases.possibility.storyElements.join(', ') || 'Open'}
- Trajectory (${(phases.trajectory.intensity * 100).toFixed(0)}%): ${phases.trajectory.storyElements.join(', ') || 'Uncharted'}
- Destiny (${(phases.destiny.intensity * 100).toFixed(0)}%): ${phases.destiny.storyElements.join(', ') || 'Unknown'}

**Active Thematic Elements**: ${thematicElements.join(', ') || 'None defined'}
**Global Story Threads**: ${state.globalThreads.join(', ') || 'None established'}
...
Weave responses that honor the current phase while hinting at possible transitions.`;
    ...
    orchestrationDirective: {
        ...
        arguments: [
            ...
            { name: 'agents', description: 'Comma-separated agent names', required: true },
        ],
        handler: (
            arena: ArenaMembrane,
            agentRegistry: Map<string, AgentReference>,
            ...
            const agents = agentNames
                .map(name => Array.from(agentRegistry.values()).find(a => a.name === name))
...
${agents.map(a => `- **${a.name}** (${a.agentId}) - Status: ${a.status}`).join('\n')}
...
**Recent Lore**:
${recentLore.map(l => `- [${l.category}] ${l.content.slice(0, 100)}...`).join('\n')}
