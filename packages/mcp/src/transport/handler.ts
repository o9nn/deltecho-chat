...
function createMethodHandlers(): MethodHandlers {
    return {
        ...
        'resources/read': (server, params) => {
            ...
            if (!uri) {
                throw { code: ErrorCodes.InvalidParams, message: 'Missing uri parameter' };
            }
        ...
        'tools/call': async (server, params) => {
            ...
            if (!fullName) {
                throw { code: ErrorCodes.InvalidParams, message: 'Missing name parameter' };
            }
...
export class ProtocolHandler {
    ...
    async handleRequest(request: MCPRequest): Promise<MCPResponse | null> {
        ...
        if (request.jsonrpc !== '2.0') {
            return this.createError(request.id, ErrorCodes.InvalidRequest, 'Invalid JSON-RPC version');
        }
        ...
        try {
            ...
            if (this.isMCPError(error)) {
                return this.createError(request.id, error.code, error.message, error.data);
            }
    ...
    async handleRawRequest(raw: string): Promise<string | null> {
        ...
        try {
            ...
            const error = this.createError(null, ErrorCodes.ParseError, 'Invalid JSON');
