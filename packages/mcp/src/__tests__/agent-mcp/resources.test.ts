...
function createMockAgentMembrane(): AgentMembrane {
    return {
        ...
        getState: vi.fn(() => ({
            ...
            transactionalMemory: [
                { id: 'tx-1', type: 'promise', content: 'Help user', status: 'pending', timestamp: Date.now() },
                { id: 'tx-2', type: 'commitment', content: 'Research topic', status: 'fulfilled', timestamp: Date.now() - 1000 },
            ],
            socialMemory: new Map([
                ['user-1', { name: 'Alice', relationship: 'friend', trustLevel: 0.9, lastInteraction: Date.now() }],
                ['user-2', { name: 'Bob', relationship: 'acquaintance', trustLevel: 0.6, lastInteraction: Date.now() - 5000 }],
            ]),
        })),
        getSocialMemory: vi.fn((contactId: string) => {
            if (contactId === 'user-1') {
                return { name: 'Alice', relationship: 'friend', trustLevel: 0.9, lastInteraction: Date.now() };
            }
            return null;
        }),
...
describe('Agent Resources', () => {
    ...
    describe('agentResources', () => {
        ...
        describe('agent://social/{contactId}', () => {
            ...
            it('should have valid schema', () => {
                ...
                expect(schema.parse({ contactId: 'user-1' })).toEqual({ contactId: 'user-1' });
    ...
    describe('listAgentResources', () => {
        ...
        it('should include social memory resources', () => {
            const resources = listAgentResources(agent);
            const socialResources = resources.filter(r => r.uri.startsWith('agent://social/'));
