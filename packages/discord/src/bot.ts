...
export class DeepTreeEchoDiscordBot {
    ...
    private async handleMessage(message: Message): Promise<void> {
        ...
        const context: DiscordMessageContext = {
            ...
            isDM: !message.guild,
            mentioned: this.client.user ? message.mentions.has(this.client.user) : false,
        ...
        this.log(`Processing message from ${context.username}: "${content.substring(0, 50)}..."`);
    ...
    private async registerSlashCommands(): Promise<void> {
        ...
        try {
            ...
            if (this.config.guildId) {
                ...
                this.log('Commands registered globally (may take up to 1 hour to propagate)');
            }
        } catch (error) {
 ...
 */
export function createDiscordBot(config: DiscordBotConfig): DeepTreeEchoDiscordBot {
