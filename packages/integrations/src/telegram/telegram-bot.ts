...
export class TelegramBot implements IPlatformIntegration {
     ...
     */
    async sendMessage(chatId: string, response: PlatformResponse): Promise<string> {
        ...
        const context: CommandContext = {
            ...
            reply: async (response) => {
                ...
                return this.sendMessage(
                    ...
                    { ...resp, replyTo: message.message_id.toString() }
                );
            },
        };
        ...
        try {
            ...
            if (response) {
                await this.sendMessage(
                    message.chat.id.toString(),
                    { ...response, replyTo: message.message_id.toString() }
                );
            }
        } catch (error) {
            this.stats.errors++;
    ...
    private toPlatformMessage(message: TelegramMessage): PlatformMessage {
        const chatType = message.chat.type === 'private' ? 'dm' :
            message.chat.type === 'group' || message.chat.type === 'supergroup' ? 'group' :
                'channel';
        ...
        return {
            ...
            author: {
                ...
                displayName: (message.from?.first_name || '') + (message.from?.last_name ? ` ${message.from.last_name}` : '') || 'Unknown',
                isBot: message.from?.is_bot,
            },
            channel: {
                id: message.chat.id.toString(),
                name: message.chat.title || message.chat.username || message.chat.first_name,
                type: chatType,
            },
    ...
    private registerDefaultCommands(): void {
        ...
        this.registerCommand({
            ...
            execute: async () => {
                ...
                return {
                    content: `📊 <b>Deep Tree Echo Status</b>\n\n` +
        ...
        this.registerCommand({
            ...
            execute: async (ctx) => {
                if (ctx.args.length === 0) {
                    return { content: 'Please provide a question. Usage: /ask <question>' };
                }
