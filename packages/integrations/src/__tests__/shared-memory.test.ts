...
describe('SharedMemoryManager', () => {
    ...
    describe('Memory Retrieval', () => {
        it('should get memory by ID', async () => {
            const stored = await manager.remember(
                MemoryType.FACT,
                'Test memory'
            );
    ...
    describe('Memory Search', () => {
        beforeEach(async () => {
            await manager.remember(MemoryType.FACT, 'User likes coffee', { userId: 'user_1' });
            await manager.remember(MemoryType.FACT, 'User likes tea', { userId: 'user_1' });
            await manager.remember(MemoryType.PREFERENCE, 'Dark mode preferred', { userId: 'user_1' });
            await manager.remember(MemoryType.FACT, 'Another user likes coffee', { userId: 'user_2' });
        });
    ...
    describe('Memory Reinforcement', () => {
        ...
        it('should throw error for non-existent memory', async () => {
            await expect(manager.reinforce('non_existent', 0.1))
    ...
    describe('User and Conversation Memories', () => {
        beforeEach(async () => {
            await manager.remember(MemoryType.FACT, 'User fact 1', { userId: 'user_1' });
            await manager.remember(MemoryType.FACT, 'User fact 2', { userId: 'user_1' });
            await manager.remember(MemoryType.CONVERSATION, 'Conv message', { conversationId: 'conv_1' });
        });
    ...
    describe('Interaction Context', () => {
        beforeEach(async () => {
            ...
            await manager.remember(MemoryType.CONVERSATION, 'Hello', { conversationId: 'conv_1' });
        });
