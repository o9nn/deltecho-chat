...
import { WebGPUInferenceEngine, DEFAULT_WEBGPU_CONFIG } from '../webgpu/index.js';
...
describe('WebGPUInferenceEngine', () => {
    ...
    describe('inference', () => {
        ...
        it('should generate text from prompt', async () => {
            ...
            expect(['completed', 'max_tokens', 'stop_sequence']).toContain(result.finishReason);
        });
        ...
        it('should emit inference events', async () => {
            ...
            const startEvents = eventListener.mock.calls.filter(c => c[0].type === 'inference_start');
            const completeEvents = eventListener.mock.calls.filter(c => c[0].type === 'inference_complete');
