...
test.describe('Deep Tree Echo Chat Integration', () => {
    ...
    test.describe('Chat Discovery', () => {
        ...
        test('AI should be able to see unread chat indicators', async ({ page }) => {
            if (existingProfiles.length < 1) {
                test.skip()
                return
            }
            ...
            const unreadBadges = page.locator('.chat-list-item .unread-badge, .chat-list-item .fresh-message-counter')
        ...
        test('AI should be able to search for chats', async ({ page }) => {
            ...
            const searchInput = page.locator('[data-testid="chat-list-search-input"], .chat-list-search input, input[placeholder*="Search"]')
            ...
            if (searchVisible) {
                ...
                console.log('Search input not visible - testing alternative search method')
    ...
    test.describe('Chat Selection', () => {
        test('AI should be able to open/select a specific chat', async ({ page }) => {
            ...
            if (chatExists) {
                ...
                const composer = page.locator('#composer-textarea, .composer-textarea, [data-testid="composer-textarea"]')
                const composerVisible = await composer.isVisible().catch(() => false)

                console.log(`AI opened chat with ${userB.name}: composer visible = ${composerVisible}`)
        ...
        test('AI should be able to send a message to an open chat', async ({
            ...
            if (chatExists) {
                ...
                if (composerVisible) {
                    ...
                    const sendButton = page.locator('button.send-button, [data-testid="send-button"], button[aria-label*="Send"]')
                    ...
                    if (sendVisible) {
                        ...
                        const messageText = await sentMessage.locator('.msg-body .text, .text').textContent().catch(() => '')
    ...
    test.describe('Chat Creation', () => {
        test('AI should be able to initiate new chat creation', async ({ page }) => {
            if (existingProfiles.length < 1) {
                test.skip()
                return
            }
            ...
            if (buttonVisible) {
                ...
                const creationDialog = page.locator('.dialog, .create-chat, [role="dialog"]')
                const dialogVisible = await creationDialog.isVisible().catch(() => false)
    ...
    test.describe('Proactive Messaging System', () => {
        ...
        test('AI should have access to trigger management', async ({ page }) => {
            ...
            if (settingsVisible) {
                ...
                const aiSettings = page.locator('.ai-settings, .proactive-settings, [data-testid="ai-settings"]')
                const aiSettingsVisible = await aiSettings.isVisible().catch(() => false)
    ...
    test.describe('Mention Detection', () => {
        test('AI should respond properly to mentions in incoming messages', async ({
            ...
            if (chatExists) {
                ...
                console.log(`AI mention detection check: mention found = ${mentionFound}`)
            }
        })
    })
