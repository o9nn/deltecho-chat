...
interface StreamingLipSyncTestContext {
  controller: {
    ...
    getCurrentMouthShape: () => { mouthOpen: number; mouthWide: number; lipRound: number }
    getFullText: () => string
    isComplete: () => boolean
    getProgress: () => {
...
async function injectTestHarness(page: Page): Promise<void> {
  await page.evaluate(() => {
    ...
    // Phoneme mapping
    const CHAR_TO_MOUTH: Record<string, { mouthOpen: number; mouthWide: number; lipRound: number }> = {
    ...
    function updateAnimation() {
      ...
      if (charIndex < currentPhraseText.length) {
        ...
        const shape = CHAR_TO_MOUTH[char] || { mouthOpen: 0.3, mouthWide: 0.2, lipRound: 0 }
    ...
    const controller = {
      start() {
        ...
        animationFrame = window.setInterval(updateAnimation, 33) as unknown as number
      },
    ...
    ;(window as unknown as { __streamingLipSyncTest: StreamingLipSyncTestContext }).__streamingLipSyncTest = {
      controller,
      events,
    }
  })
...
async function waitForEvent(
  ...
  while (Date.now() - startTime < timeout) {
    const event = await page.evaluate((type) => {
      const ctx = (window as unknown as { __streamingLipSyncTest: StreamingLipSyncTestContext }).__streamingLipSyncTest
...
test.describe('Streaming Lip-Sync Controller', () => {
  ...
  test('should process streaming text chunks incrementally', async ({ page }) => {
  ...
  test('should detect phrase boundaries and queue phrases', async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT)
    ...
    await page.evaluate(() => {
      const ctx = (window as unknown as { __streamingLipSyncTest: StreamingLipSyncTestContext }).__streamingLipSyncTest
      ctx.controller.processChunk({ content: 'First sentence. Second sentence! Third?', isComplete: false })
    })
  ...
  test('should generate mouth shapes during animation', async ({ page }) => {
    ...
    await page.evaluate(() => {
      const ctx = (window as unknown as { __streamingLipSyncTest: StreamingLipSyncTestContext }).__streamingLipSyncTest
      ctx.controller.start()
      ctx.controller.processChunk({ content: 'Hello world.', isComplete: true })
    })
    ...
    const firstMouthEvent = mouthEvents[0] as { type: string; data: { mouthShape: { mouthOpen: number; mouthWide: number; lipRound: number } } }
  ...
  test('should emit speaking_start and speaking_end events for each phrase', async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT)
  ...
  test('should handle rapid streaming without losing data', async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT)
    ...
    for (const token of tokens) {
      await page.evaluate((content) => {
        const ctx = (window as unknown as { __streamingLipSyncTest: StreamingLipSyncTestContext }).__streamingLipSyncTest
        ctx.controller.processChunk({ content: content + ' ', isComplete: false })
      }, token)
    ...
    expect(fullText.trim()).toBe('The quick brown fox jumps over the lazy dog.')
  ...
  test('should return to rest position after stream completes', async ({ page }) => {
    test.setTimeout(TEST_TIMEOUT)
  ...
  test('should track progress accurately', async ({ page }) => {
    ...
    await page.evaluate(() => {
      const ctx = (window as unknown as { __streamingLipSyncTest: StreamingLipSyncTestContext }).__streamingLipSyncTest
      ctx.controller.processChunk({ content: 'First. Second. Third.', isComplete: true })
    })
