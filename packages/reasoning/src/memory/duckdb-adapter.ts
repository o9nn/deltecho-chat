    ...
    public async initialize(): Promise<void> {
        ...
        try {
            ...
            if (typeof Worker === 'undefined') {
                log('Web Worker API not available. Skipping DuckDB WASM init (mock mode).');
                return;
            }
...
...
    ...
    public async storeAtom(atom: {
        ...
        await this.drizzleDb.insert(atoms)
            .values({
                id: atom.id,
                type: atom.type,
                name: atom.name,
    ...
    public async findAtoms(type?: string, namePattern?: string): Promise<any[]> {
        ...
        return await this.drizzleDb.select()
            .from(atoms)
    ...
    private async loadFromPersistence(): Promise<void> {
        ...
        try {
            ...
            if (atomBuffer) {
                ...
                // Load into table
                await this.execute(`INSERT INTO atoms SELECT * FROM read_parquet('atoms.parquet')`);
                log('Loaded atoms from persistence.');
            }

            if (linkBuffer) {
                await this.db.registerFileBuffer('links.parquet', linkBuffer);
                await this.execute(`INSERT INTO links SELECT * FROM read_parquet('links.parquet')`);
     ...
     */
    private async saveToHostStorage(filename: string, buffer: Uint8Array): Promise<void> {
        // Node.js environment
     ...
     */
    private async loadFromHostStorage(filename: string): Promise<Uint8Array | null> {
        // Node.js
        if (typeof process !== 'undefined' && process.versions && process.versions.node) {
            if (typeof require !== 'undefined') {
                try {
