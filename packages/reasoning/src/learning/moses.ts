import { Atom, AtomType, Link, Node } from '../atomspace/atom.js';
import { TruthValue } from '../atomspace/truth-value.js';
...
export class MosesEngine {
    ...
    public async evolve(
        ...
        while (population.length < this.config.populationSize && population.length > 0) {
        ...
        for (let gen = 0; gen < this.config.generations; gen++) {
            ...
            // Elitism
            for (let i = 0; i < this.config.elitismCount && i < population.length; i++) {
        ...
        if (atom.isLink()) {
            if (atom.outgoing.length > 0) {
                ...
                if (Math.random() > 0.5 && newOutgoing.length > 1) {
                    ...
                    [newOutgoing[idx1], newOutgoing[idx2]] = [newOutgoing[idx2], newOutgoing[idx1]];
                }
    ...
    private crossover(parent1: Atom, parent2: Atom): Atom {
        ...
        if (parent1.isLink() && parent2.isLink()) {
            ...
            if (parent1.outgoing.length > 0) {
                ...
                const subtree = parent2.outgoing[Math.floor(Math.random() * parent2.outgoing.length)];
                if (subtree) {
    ...
    private calculateComplexity(atom: Atom): number {
        ...
        if (atom.isLink()) {
            return 1 + atom.outgoing.reduce((acc, child) => acc + this.calculateComplexity(child), 0);
        }
        return 1;
    }
}
