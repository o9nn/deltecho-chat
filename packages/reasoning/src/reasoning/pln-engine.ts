...
import { Node, Link, AtomType } from '../atomspace/atom.js';
...
export class PLNEngine {
    ...
    public deduce(): number {
        const inheritanceLinks = this.atomSpace.getByType(AtomType.InheritanceLink) as Link[];
        let newAtoms = 0;
        ...
        for (const ab of inheritanceLinks) {
            ...
            for (const bc of bcLinks) {
                ...
                const existing = this.atomSpace.getAtom(`${AtomType.InheritanceLink}(${A.id},${C.id})`);
                if (!existing) {
