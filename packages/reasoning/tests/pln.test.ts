import { AtomSpace, AtomType, PLNEngine, createTruthValue } from '../src/index.js';
...
describe('PLNEngine', () => {
    ...
    it('should perform deduction', () => {
        ...
        as.link(AtomType.InheritanceLink, [sokrates, human], createTruthValue(1.0, 1.0));
        // Human -> Mortal (TV: 1.0, 1.0)
        as.link(AtomType.InheritanceLink, [human, mortal], createTruthValue(1.0, 1.0));

        const newAtomsCount = pln.deduce();
        expect(newAtomsCount).toBe(1);

        const deducerResult = as.getAtom(`${AtomType.InheritanceLink}(${sokrates.id},${mortal.id})`);
