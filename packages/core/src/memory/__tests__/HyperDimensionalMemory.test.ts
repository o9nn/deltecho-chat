...
describe('HyperDimensionalMemory', () => {
  ...
  describe('recallMemories', () => {
    beforeEach(() => {
      hyperMemory.storeMemory('msg_1', 'I love programming in TypeScript', Date.now(), 1.0);
      hyperMemory.storeMemory('msg_2', 'TypeScript is great for large projects', Date.now(), 1.0);
      hyperMemory.storeMemory('msg_3', 'Python is excellent for data science', Date.now(), 1.0);
      hyperMemory.storeMemory('msg_4', 'JavaScript runs in browsers', Date.now(), 1.0);
    });
  ...
  describe('memory associations', () => {
    it('should build associations between related memories', () => {
      hyperMemory.storeMemory('msg_1', 'The cat sat on the mat', Date.now(), 1.0);
      hyperMemory.storeMemory('msg_2', 'The cat played with yarn', Date.now(), 1.0);
      hyperMemory.storeMemory('msg_3', 'Dogs like to fetch balls', Date.now(), 1.0);
  ...
  describe('temporal indexing', () => {
    it('should index memories by time', () => {
      ...
      hyperMemory.storeMemory('msg_2', 'Yesterday message', now - 86400000, 1.0);
  ...
  describe('emotional weighting', () => {
    it('should weight emotionally significant memories higher', () => {
      hyperMemory.storeMemory('msg_1', 'Important emotional event', Date.now(), 3.0);
  ...
  describe('edge cases', () => {
    ...
    it('should handle special characters', () => {
      expect(() => {
        hyperMemory.storeMemory('msg_1', 'Hello! @#$%^&*() 你好 🎉', Date.now(), 1.0);
      }).not.toThrow();
    });
  });
});
