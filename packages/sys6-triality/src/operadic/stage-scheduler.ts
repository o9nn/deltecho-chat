import { OperadicState, OrchestrationEvent, StreamType } from '../types/index.js';

export class StageScheduler {
    // Map of cycle step (0-29) to list of event templates
    private schedule: Map<number, Omit<OrchestrationEvent, 'timestamp'>[]> = new Map();
    ...
    private createEventTemplate(id: number, type: string, targetStream: StreamType, priority: number): Omit<OrchestrationEvent, 'timestamp'> {
        return {
            id: `sys6-evt-${id}-${type}`,
            type,
            targetStream,
