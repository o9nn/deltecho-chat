...
- `pnpm start:bot` is the standalone DeltaChat bot (`bin/deltecho-bot.ts`). It is not the orchestrator and is not started by `start:orchestrator`. When `DELTECHO_AUTONOMY_STORAGE_PATH` names an existing filesystem store containing `deepTreeEchoBotMemories.json`, the bot retrieves relevant memories for each direct message and persists the completed turn. Unset, missing, invalid, and keyless stores are skipped without creating files. `DELTECHO_BOT_PERSONALITY` optionally overrides the bot's personality clause while preserving its Deep Tree Echo identity.
