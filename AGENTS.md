...
- `pnpm start:bot` is the standalone DeltaChat bot (`bin/deltecho-bot.ts`). It is not the orchestrator and is not started by `start:orchestrator`. When `DELTECHO_AUTONOMY_STORAGE_PATH` is a non-empty path to an existing filesystem RAG store (live `deepTreeEchoBotMemories.json`), the bot opens it with `createIfMissing: false`, retrieves relevant memories before each Claude call, and persists the user turn plus bot reply via `RAGMemoryStore.storeMemory`. Unset, empty, missing, invalid JSON, or no RAG key skips memory for the process; the bot still answers and creates no store files. The message path does not call MemoryLever `dream`/`apply`. Optional `DELTECHO_BOT_PERSONALITY` overrides the personality clause of the system prompt; default identity remains Deep Tree Echo.
