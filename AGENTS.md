...
- `pnpm start:bot` is the standalone DeltaChat bot (`bin/deltecho-bot.ts`). It is not the orchestrator and is not started by `start:orchestrator`. Like `memory:lever`, its `npx ts-node` wrapper fails on this Node ESM tree; use `npx tsx bin/deltecho-bot.ts`.
- The bot honors `DELTECHO_AUTONOMY_STORAGE_PATH`. At startup it opens that existing filesystem RAG store with `createIfMissing: false`, then on each inbound 1:1 message it retrieves live memories, injects them into that call's system prompt, and writes the user turn plus its reply back with `RAGMemoryStore.storeMemory`. It never calls MemoryLever `dream`/`apply`; consolidation stays scheduled hygiene. Unset path, missing directory, vector-only directory, or unparseable RAG JSON logs one skip reason (`unset_path`, `missing_store`, `no_rag_keys`, `missing_or_invalid`), leaves the disk untouched, and the bot answers without memory. Bot memory logs carry counts and reason codes only, never memory text.
- The bot honors `DELTECHO_BOT_PERSONALITY`. It replaces the personality clause of the system prompt only; capabilities, safety guidelines, and the retrieved-memory section are unchanged. Unset or blank keeps the Deep Tree Echo identity.
- The reply-context assembly the bot uses lives in `deep-tree-echo-core/bot` (`openBotMemory`, `BotMemory.retrieve`, `BotMemory.rememberTurn`, `buildReplyContext`). It is Node-only — keep it off the browser barrels — and is covered by `packages/core/src/bot/__tests__` without DeltaChat, Anthropic, or network.
- `DELTECHO_AUTONOMY_STORAGE_PATH` must already contain live RAG key `deepTreeEchoBotMemories` (`deepTreeEchoBotMemories.json` on disk). Vector-only directories (`vectorMemoryStore_memories`) and desktop settings JSON are a different store; neither the scheduled lever nor the bot opens them, and both skip with `no_rag_keys`.
